...
const GAMEPASS_DEDUCTION_RATE = 0.3;
const REFERRAL_BONUS_RATE = 0.1;
const segments = [
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    if (
      ...
      window.location.pathname.endsWith("referal.html")
    ) {
      window.location.href = "join.html";
    }
  }
...
function updateDashboardUI(userData) {
  ...
  if (profileUsername)
    profileUsername.textContent = userData.displayName || "User";
  ...
  const profileReferralCode = document.getElementById(
    "profile-user-referral-code"
  );
  if (profileReferralCode)
    profileReferralCode.textContent = userData.userReferralCode;

  const referralCodeDisplay = document.getElementById("referral-code-display");
  if (referralCodeDisplay)
    referralCodeDisplay.textContent = userData.userReferralCode;

  const profileReferredBy = document.getElementById("profile-referred-by");
  if (profileReferredBy)
    profileReferredBy.textContent = userData.referredBy || "N/A";
}
...
if (wheelCanvas) {
  ...
  function spinWheel() {
    ...
    const landingAngle =
      winningSegmentIndex * segmentsAngle + segmentsAngle / 2;
  ...
  function watchAd() {
    ...
    // Listen for Monetag ad complete (replace with their actual event if available)
    document.addEventListener("monetagAdComplete", () => {
      grantFreeSpin();
    });

    // Fallback if no event → manual timer
    setTimeout(() => {
      grantFreeSpin();
    }, 15000); // 15s to match typical video ads
  }

  function grantFreeSpin() {
  ...
  // --- Fix Spin Wheel Accuracy ---
  function getLandingAngleForSegment(segmentIndex) {
    const segmentsAngle = 360 / segments.length;
    const offsetToPointer = -segmentsAngle / 2; // center segment under pointer
    return segmentIndex * segmentsAngle + segmentsAngle / 2 + offsetToPointer;
  }
...
if (copyReferralBtn) {
  copyReferralBtn.addEventListener("click", () => {
    const referralCodeText = document.getElementById(
      "referral-code-display"
    ).textContent;
    navigator.clipboard
      .writeText(referralCodeText)
      .then(() => {
